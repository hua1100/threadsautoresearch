...
<feed xmlns:yt="http://www.youtube.com/xml/schemas/2015"
  ...
  <title>every.to</title>
  <entry>
    <yt:videoId>abc123</yt:videoId>
    <title>AI Tools 2026</title>
    <published>2099-01-01T10:00:00+00:00</published>
    <media:group>
      <media:description>A great video about AI tools</media:description>
    </media:group>
  </entry>
</feed>"""
...
def test_fetch_recent_videos_returns_list():
    ...
    assert videos[0]["channel"] == "every.to"


def test_fetch_recent_videos_filters_old():
    old_rss = SAMPLE_RSS.replace("2099-01-01", "2020-01-01")
    mock_resp = MagicMock()
    mock_resp.status_code = 200
    mock_resp.text = old_rss
