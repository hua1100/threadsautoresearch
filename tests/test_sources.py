...
def test_fetch_recent_videos_returns_list():
    ...
    with patch("orchestrator.sources.youtube.requests.get", return_value=mock_resp):
        with patch("orchestrator.sources.youtube.YOUTUBE_API_KEY", "fake-key"):
...
def test_fetch_x_content_plain_text(tmp_path):
    ...
    results = fetch_x_content(messages, str(curated_file))
    assert len(results) == 2
    assert results[0]["source"] == "telegram"
    assert "karpathy" in results[0]["text"]


def test_fetch_x_content_youtube_url_with_transcript(tmp_path):
    curated_file = tmp_path / "x_curated.md"
    curated_file.write_text("## 策展內容\n")

    messages = ["https://www.youtube.com/watch?v=dQw4w9WgXcQ 這個影片很棒"]

    with patch("orchestrator.sources.x_curated._fetch_transcript", return_value="This is a test transcript"):
        results = fetch_x_content(messages, str(curated_file))

    assert len(results) == 1
    assert results[0]["source"] == "telegram/youtube"
    assert results[0]["video_id"] == "dQw4w9WgXcQ"
    assert "逐字稿" in results[0]["text"]
    assert "This is a test transcript" in results[0]["text"]
...
def test_fetch_x_content_appends_to_curated_file(tmp_path):
    ...
    messages = ["新的好內容"]
    fetch_x_content(messages, str(curated_file))
